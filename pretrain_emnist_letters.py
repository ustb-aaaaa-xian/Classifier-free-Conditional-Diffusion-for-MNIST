...
class DDPM(nn.Module):
	...
	def sample(self, n_sample, size, device, guide_w=0.0):
		...
		c_i = torch.arange(0,26).to(device)
...
def emnist_letters_pretrain():
	...
	save_dir = f'./data/pretrain_emnist_letters/'  # emnist_letters预训练保存
	if not os.path.exists(save_dir):
		print("save_path not exists,try to mkdirs")
		os.makedirs(save_dir)
	# data
	dataset = EMNIST(root = "./data",split = "letters",transform = tf,train = True,download = True)
	dataset_test = EMNIST(root = "./data",split = "letters",transform = tf,train = False,download = True)
	...
	for ep in range(n_epoch):
		...
		for (x,c) in pbar:  # x是data,c是label;训练/微调
			# print(x.shape)
			optim.zero_grad()
			x = x.transpose(-1, -2) # emnist数据是旋转回来才正
			# print(x.shape)
			x = x.to(device)
			c = c -1
			c = c.to(device)
		...
		ddpm.eval()
		if not (ep % 10 ==0 or ep == 0 or ep >=n_epoch - 5) :
			continue
		with torch.no_grad():
			...
			if True:
				for w_i, w in enumerate(ws_test):
					...
					grid = make_grid((x_all - x_all.min()) / (x_all.max() - x_all.min()), nrow=n_classes)
...
if __name__ == "__main__":
	s_time = time.time()
	emnist_letters_pretrain()
	end_time = time.time()
	print(f"{end_time - s_time}s has passed after pretrain emnist_letters for {create_parser().n_epoch} epochs.")
