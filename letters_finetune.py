...
from utils.letters_dataloader import letters_dataloader
import numpy as np
import os
import time
import argparse
...
class ContextUnet(nn.Module):
	def __init__(self, in_channels, n_feat=256, n_classes=26):
...
class DDPM(nn.Module):
	...
	def sample(self, n_sample, size, device, guide_w=0.0):
		...
		c_i = torch.arange(0, 26).to(device)  # context for us just cycles throught the mnist labels
...
def train_data():
	...
	save_dir = f"./Generate_save/letters_finetune/ocr{ocr}/{dataset_name}_{num_each_class}/"
	...
	ddpm.load_state_dict(torch.load('./data/pretrain_emnist_letters/model_40.pth'))
	ddpm.to(device)
	# print(ddpm)
	...
	# 尝试多种数据，改写每次的dataset
	dataset = letters_dataloader(data_path = f"./data/ocr{ocr}/{dataset_name}_patch_letters",transform = tf,num_each_class=num_each_class)
		...
		with torch.no_grad():
			...
			if True:
				for w_i, w in enumerate(ws_test):
					...
					grid = make_grid((x_all - x_all.min()) / (x_all.max() - x_all.min()), nrow=n_classes) # n_row改成n_classes
			...
			if True:
				...
				for w_i, w in enumerate(ws_test):  # 如果只想生成 w = 2.0 的话就可以不用for循环
					...
					x_gen, x_gen_store = ddpm.sample(n_sample, (3, 28, 28), device,
													 guide_w=w)  # 这个sample函数是不是也可以重写，因为根本不需要生成过程的保存，只需要保存生成效果(没必要，生成中间结果的保存并不占用时间)
					...
					for i in range(x_gen.shape[0]):
						...
						if height<28 or width <28:
							img = transforms.Resize((height, width), interpolation=transforms.InterpolationMode.NEAREST)(
							img)
						else:
							img = transforms.Resize((height, width), interpolation=transforms.InterpolationMode.BICUBIC)(
							img)  # 变成原始长条状
						j = 0
						save_path = generate_save_dir + f"image_w{w:.1f}_{chr(i % n_classes + 65) }_{j}.png"
						while os.path.exists(save_path):
							j += 1
							save_path = f"{generate_save_dir}image_w{w:.1f}_{chr(i % n_classes+65)}_{j}.png"
						save_image(img, save_path)
						print('saved image at ' + save_path, end="\r")
					generate_e_time = time.time()
					print(
						f"{generate_e_time - generate_s_time}s has passed when generating {n_sample // n_classes} batches of {n_sample} characters of ocr{ocr} model_id {dataset_name}.")
		end_time2 = time.time()
...
"""
	字母的微调数据并不存在全部类别，所以生成以及保存可以简化，只生成与展示存在的类别。
	目前的改动有：
		（1）对于展示生成与真实图片的拼接时，只展示存在的类别（主要针对生成图像）
	TODO：
		对于最后展示时的真实数据，只能在最后一个batch里采用，计划将展示的real图像进行对应。。。
"""
def train_data_new():
	start_finetune_time = time.time()
	n_epoch = create_parser().n_epoch
	...
	save_dir = f"./Generate_save/letters_finetune/ocr{ocr}/{dataset_name}_{num_each_class}_new/"
	ws_test = [0.0, 0.5, 2.0]  # strength of generative guidance
	...
	if not os.path.exists(save_dir):
		...
		os.makedirs(save_dir)
	# 尝试多种数据，改写每次的dataset
	dataset = letters_dataloader(data_path = f"./data/ocr{ocr}/{dataset_name}/{dataset_name}_patch_letters",transform = tf,num_each_class=num_each_class)
	# 得到新c_exist
	...
	for ep in range(n_epoch):
		...
		with torch.no_grad():
			...
			if True:
				for w_i, w in enumerate(ws_test):
					x_gen, x_gen_store = ddpm.sample_for_exist(n_sample, (3, 28, 28), device, guide_w=w,c_exist=c_exist)
					...
					# 			idx = torch.squeeze((c == k).nonzero())[j]  # k 被广播机制去与c做等运算，j是行数。
					# 		except:
					# 			m = torch.randint(0, n_classes, (1,)) # 改成有的标签。。。
					# 			idx = (0 + m.item()) % len(c)
					# 		x_real[k + (j * len(c_exist))] = x[idx]  # 随机展示
					...
					x_all = torch.cat([x_gen, x_real])
					grid = make_grid((x_all - x_all.min()) / (x_all.max() - x_all.min()), nrow=len(c_exist)) # n_row改成n_classes
					save_image(grid, save_dir + f"image_ep{ep}_w{w}.png")
					print('saved image at ' + save_dir + f"image_ep{ep}_w{w}.png")
			...
			if True:
				...
				for w_i, w in enumerate(ws_test):  # 如果只想生成 w = 2.0 的话就可以不用for循环
					...
					generate_e_time = time.time()
					print(
						f"{generate_e_time - generate_s_time}s has passed when generating {n_sample // len(c_exist)} batches of {n_sample} characters of ocr{ocr} model_id {dataset_name}.")
		end_time2 = time.time()
		# print(f"this epoch has passed {end_time2-start_time}s. forward train has used {end_time1-start_time}s, generating(sampling) has used {end_time2 - end_time1}s.")
...
def create_parser():  # 这样写是可行的
	...
	parser.add_argument("--dataset_name", default="GGASVUPO", type=str)  # 数据集名称，设置重点
...
if __name__ == "__main__":
	...
	end_time = time.time()
	print(
		f"{end_time - s_time}s has passed after 100 epoches finetune and  {create_parser().n_sample_mul} batches of {create_parser().n_classes} characters[n_sample = {create_parser().n_sample_mul * create_parser().n_classes}] generating.")
