...
def train_mnist():
    ...
    save_dir = './data/diffusion_outputs_mnist64_3_channel_/'
    ...
    for ep in range(n_epoch):
        ...
        time2 = time.time()
        if ep %10 == 0:
            print(f"this epoch has passed {time2-time1}s.")
        ddpm.eval()
        ...
        with torch.no_grad():
            ...
            for w_i, w in enumerate(ws_test):
                ...
                save_image(grid, save_dir + f"image_ep{ep}_w{w}.png") # 这用的是+,所以前面的save_dir后面还要有一个"/"
                gene_end_time = time.time()
                print('saved image at ' + save_dir + f"image_ep{ep}_w{w}.png")
                print(f"{gene_end_time - gene_start_time}s has passed when generating {n_sample//n_classes} batches of {n_sample} samples,w={w}")
                if   ep == int(n_epoch - 1):
        ...
        if ep %10  == 0 or ep == int(n_epoch - 1) or ep == 0 :
...
def create_parser(): # 这样写是可行的
    ...
    parser.add_argument("--dataset_name", default = "1HZZ0HMD",type=str)  # 这个也用不到
    parser.add_argument("--num_each_class", type=int, default=1)  # 预训练用不到,是不是可以设置成str？然后如果用全部就all
