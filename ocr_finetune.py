...
class DDPM(nn.Module):
    ...
    def sample_for_paticular(self,n_sample,size,device,guide_w = 2.0,c = 0):
        x_i = torch.randn(n_sample, *size).to(device)
        c_i = torch.ones(n_sample) * c
...
def train_data():
    ...
    save_dir = f'./output/ocr{ocr}_3_channel/ocr_finetune_{dataset_name}_{num_each_class}_da_/'
    ...
    ddpm.load_state_dict(torch.load('./data/diffusion_outputs_mnist_3_channel/model_49.pth'))
    ddpm.to(device)
    # print(ddpm)
    # 自己写的 ， 加入预训练权重
     # 预训练好的MNIST权重
    ...
    for ep in range(n_epoch):
        ...
        for x, c in pbar: # x是data,c是label;训练/微调
            optim.zero_grad()
            x = x.to(device)
            # 此条语句只针对微调数据为白底黑字的情况，因为MNIST本身就是黑底白字的
            # x = (x*-1)+1
            c = c.to(device)
            loss = ddpm(x, c)
            loss.backward()
        ...
        if ep == n_epoch - 1:
            ...
            print(f"after 100 epochs finetune ,{end_time_100_epoch - start_time} has passed.")
        with torch.no_grad():
            ...
            if True:
                generate_save_dir = f"./Generate_save/ocr{ocr}_3_channels/{dataset_name}_{num_each_class}/"
                height,width = 110,70
                for w_i, w in enumerate(ws_test): # 如果只想生成 w = 2.0 的话就可以不用for循环
                    generate_s_time = time.time()
                    x_gen, x_gen_store = ddpm.sample(n_sample, (3, 28,28 ), device, guide_w=w) # 这个sample函数是不是也可以重写，因为根本不需要生成过程的保存，只需要保存生成效果(没必要，生成中间结果的保存并不占用时间)
                    ...
                    print(f"{generate_e_time - generate_s_time}s has passed when generating {n_sample // n_classes} batches of {n_sample} characters of ocr{ocr} model_id {dataset_name}.")
        end_time2 = time.time()
