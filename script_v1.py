...
from utils.dataloader import dataloader_v1,dataloader_ocr
import numpy as np
import os
...
def train_data():
    ...
    # ddpm.load_state_dict(torch.load('logs/training.pt'))
    ddpm.to(device)
    print(ddpm)
    # 自己写的 ， 加入预训练权重
     # 预训练好的MNIST权重
    ...
    # dataset = dataloader_v1(data_path = "data/masks",transform  = tf)
    # 直接用点阵数据进行预训练，先看什么时候能生成比较合适的
    dataset = dataloader_ocr("./data/P0X8CM8Q_pacth_ocr",transform = tf)
...
if __name__ == "__main__":
    ...
    x = x.to(device)
    c = torch.ones((1,4)).type(torch.long)
    c = c.to(device)
    ddpm(x,c)